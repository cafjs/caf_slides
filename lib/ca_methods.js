...
exports.methods = {
    ...
    async __ca_pulse__() {
        ...
        if (this.state.isAdmin) {
            ...
            const $$ = this.$.sharing.$;
            if ($$.presentations) {
                const url = $$.presentations.get(this.__ca_getName__());
    ...
    async changePage(oldPage, newPage, step) {
        const now = (new Date()).getTime();
        ...
        this.$.log && this.$.log.debug(
            `<<< Changing page time: ${now} old page: ${oldPage}` +
                ` new page: ${newPage} step: ${step}`
        );
        const current = this.state.currentPage || {};
        this.state.currentPage = {...current, num: newPage, step};
        if ((oldPage !== newPage) || !this.state.currentPage.time) {
            this.state.currentPage.time = now;
        }
        notifyWebApp(this, this.state);
        return this.getState();
    },
